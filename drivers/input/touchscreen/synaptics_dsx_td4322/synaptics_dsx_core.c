...
static struct synaptics_rmi4_data *p_rmi4_data = NULL;//wufei:Fixed the bug of probabilistic interruption after TP power-off
...
/*
	// 开中断
	if (rmi4_data->irq > 0 && !rmi4_data->irq_enabled) {
		...
		bool enable, bool attn_only)
{
	...
	if (enable) {
		...
		rmi4_data->irq_enabled = true; // 表示中断已经初始化，不表示中断是否使能
	} else {
		if (rmi4_data->irq_enabled) {
			disable_irq(rmi4_data->irq);
		...
		bool get)
{
	...
	const struct synaptics_dsx_board_data *bdata =
			rmi4_data->hw_if->board_data;
	//TP_LOGE("synaptics_rmi4_get_reg get=%d\n", get);
	if (!get) {
		retval = 0;
		goto regulator_put;
	}
...
//wufei:Fixed the bug of probabilistic interruption after TP power-off
bool tp_irq_disable;
//disable: 0-enable,1-disable
int synaptics_rmi4_disable_irq(bool disable){
	if(p_rmi4_data == NULL || !p_rmi4_data->irq_enabled){
		KERNEL_ERROR("p_rmi4_data is NULL\n");
		return -1;
	}
	//KERNEL_ERROR("tp_irq_disable=%d, disable=%d\n", tp_irq_disable, disable);
	if(tp_irq_disable != disable){
		if(disable == true){
			disable_irq(p_rmi4_data->irq);
		}else{
			enable_irq(p_rmi4_data->irq);
		}
		tp_irq_disable = disable;
	}
		...
		bool enable)
{
	...
	//KERNEL_ERROR("================[kevin]synaptics_rmi4_enable_reg()enable=%d=rmi4_data->irq_enabled=%d=========\n", enable, rmi4_data->irq_enabled);
	if (!enable) {
		retval = 0;
...
/*
	if (rmi4_data->irq > 0 && rmi4_data->irq_enabled) {
		rmi4_data->irq_enabled = true;
		//enable_irq(rmi4_data->irq);
		synaptics_rmi4_disable_irq(!enable);
	}
*/
	return 0;
...
*/
	if (rmi4_data->ts_pinctrl) {
		retval = pinctrl_select_state(rmi4_data->ts_pinctrl, rmi4_data->pinctrl_state_suspend);
		if (retval < 0) {
			TP_LOGE("Failed to select %s pinstate %d\n", "pmx_ts_suspend", retval);
		}
	}
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	tp_irq_disable = false;//wufei:Fixed the bug of probabilistic interruption after TP power-off
	p_rmi4_data = rmi4_data;//wufei:Fixed the bug of probabilistic interruption after TP power-off
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	if (ret < 0) {
		TP_LOGE("Failed to enable regulators ret=%d\n", ret);
		goto err_enable_reg;
	}
