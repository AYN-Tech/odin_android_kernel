...
int gt1x_suspend(void)
{
	...
	gpio_direction_output(128, 0);//jeffery add 20200825, powerdown HUB, system can enter suspend
#if GTP_PROXIMITY
	if (gt1x_ps_dev && gt1x_ps_dev->enabled) {
...
int gt1x_resume(void)
{
	...
	gpio_direction_output(128, 1);//jeffery add 20200825, system wakeup, powerup HUB
#if GTP_PROXIMITY
	if (gt1x_ps_dev && gt1x_ps_dev->enabled) {
