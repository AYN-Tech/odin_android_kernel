...
int virtual_touch_printk(int state)
{
    int count = 0;
    struct list_head *slot_list, *n;
    slot_node_t node = NULL;

    if (list_empty_careful(&hub_information->algorithm_head)) {
        printk("%s \r\n", "HZY root head is empty");
        return 0;
    }

    list_for_each_safe(slot_list, n, &hub_information->algorithm_head) {
        node = list_entry(slot_list, _slot_node_t, list);
        printk("HZY ----- origin %d unite %d state %s source %d", node->slot_original, node->slot_unite,
            state ? state == VIRTUAL_MOVE ? "move" : "down" : "up", node->slot_source);
    }
    printk("HZY --- ");

    return count;
}
EXPORT_SYMBOL(virtual_touch_printk);
...
int slot_conver_mechanism(void *identity, int slot, int state, int source)
{
    ...
    //virtual_touch_printk(state);
    // printk("%s", buf);

    //printk("HZY ----- origin %d unite %d state %s source %d", slot, hub_information->slot_unite,
            //state ? state == VIRTUAL_MOVE ? "move" : "down" : "up", source);
    //if (list_empty_careful(&hub_information->identity_head)) {
        //printk("HZY root head is empty");
    //}
